...
in_['beam_dir']            = 'save_beam/' + in_['tag'] + '/parkfield_bartlett_cooked_v3' # where do you want to save ?

tt = {} # time inputs
tt['year']                 = 2002 # year, no loop over year to remain simple ... run the code twice if overlap over 2 years
tt['day']                  = list(range(150,155)) # list of julian days
tt['t_0']                  = 0*60*60 # in sec, from first sample of the file (t0 for every day)
tt['t_win']                = 1*60*30 #1*60*60 # in sec, entire trace if None    #1 h de window
tt['t_inc']                = 1*60*30  #1*60*60 # in sec ? time from one window to the next   #overlap -> ici pas d'overlap
...
bb['U']                    = np.linspace(-0.3,0.3,91)*10**-3 #np.linspace(-1.3,1.3,101)*10**-3 # s/m
...
pl = {} # plot inputs
pl['fn']                   = [1.05] # list of freq to plot
pl['fw']                   = 4.**(1/6) # float, from fn[i]/fw to fn[i]*fw , if fw=2**(1/2) (an octave),  if fw=2**(1/6) (1/3 of an octave) # 0= full scale,# 1= monochomatic
pl['map']                  = False # plot station map or not ..
...
use_pre_stacked = False # mettre False si doit utiliser des versions stackées sur la journée !!! 
