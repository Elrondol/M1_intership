...
data_path                = './data_10.0hz' # archive to process
...
in_['recipe']            = ['_filter','_clipping'] # the list of function/method you want to apply
...
arg_filter = {
     'type'  : 'bp' # bp, hp or lp
    ,'f1'    : 0.5 # lower corner freq for bp and lp
    ,'f2'    : 2.0 # higher corner freq for bp and hp
    ,'order' : 4.0 # filter order
    ,'taper' : 0.01 # between 0 (off) and 1 (hann window)
}

arg_clipping = {
     'treshold' : 3.5 # treshold * std
    ,'replace'  : 3.5 # replace * std
    ,'niter'    : 0 # if 0 then it converges...
}
