...
in_['ch']             = ['BH'] # ['ch1','ch2',...] It will try in this order. Better to start from the lower sampling rate ['LH','BH','HH'...]
in_['cmp']            = ['Z'] # ['cmp1','cmp2',...]
...
in_['pp']                = {} # always '{}'
in_['pp']['freq']        = 40. # [hz], freq. to which data are decimated [hz] => output sampling rate
...
############### RUN FOR DATES
list_year                  = [2002] # list of targeted year

for iyear in list_year : # loop over a range of dates
    in_['day1']=UTCDateTime(iyear,8,13) # date 1
    in_['day2']=UTCDateTime(iyear,8,13) # date 2
